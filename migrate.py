...
class User( db.model ):
    ...
    task_id = db.Column(db.Integer, db.ForeignKey('task.task_id'), nullable=False) # 這句看以後要不要改
...
class Task( db.model ):
    __tablename__ = 'task'
    task_id = db.Column(db.String, primary_key = True, unique = True, nullable = False)
    title = db.Column(db.String, unique = True, nullable = False)
    content = db.Column(db.String(64), unique = False, nullable = False)
